...
@Autonomous(name = "AutonomousAClose")
public class AutonomousAClose extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) {
            straightFowardA(25, 1);
            turnHeadingFoward(1);
            straightFowardB(10, 1);
            turnHeadingtoShoot(1);
            shootArtifact(1);
        }
    }
    ...
    public void turnHeadingFoward(double power) {
        while (opModeIsActive() && angle <=120) {
            m_fl.setPower(power);
            m_fr.setPower(-power);
            m_br.setPower(-power);
            m_bl.setPower(power);
    ...
    public void straightFowardB(double inches, double power) {
        int TICKSDISTANCE = (int) (inches * TICKS_PER_INCH);
    ...
    public void turnHeadingtoShoot(double power) {
        while (opModeIsActive() && angle <=120) {
            m_fl.setPower(power);
            m_fr.setPower(-power);
            m_br.setPower(-power);
